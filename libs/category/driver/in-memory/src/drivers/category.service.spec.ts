...
describe('@daffodil/category/driver/in-memory | DaffInMemoryCategoryService', () => {
  ...
  describe('get | getting a single category by ID', () => {
    ...
    it('should send a get request', () => {
      ...
      categoryService.get({ id: mockCategory.id, page_size: 12, kind: DaffCategoryPageRequestKind.ID }).subscribe(categoryResponse => {
  ...
  describe('getByUri | getting a single category by URL', () => {
    ...
    it('should send a get request', () => {
      ...
      categoryService.getByUri({ uri: mockCategory.id, page_size: 12, kind: DaffCategoryPageRequestKind.URI }).subscribe(categoryResponse => {
        expect(categoryResponse).toEqual(jasmine.objectContaining({
          category: mockCategory,
          products: mockProducts,
        }));
